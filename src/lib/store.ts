  ...
  persist(
    ...
    }),
    {
      ...
      merge: (persisted, current) => {
        ...
        const withEntity = <T extends { entityId?: string }>(
          items: T[] | undefined,
          fallback: T[],
        ): T[] =>
          (items ?? fallback).map((item) => ({
            ...item,
            entityId: item.entityId || EID.edu,
          }));
        return {
          ...current,
          ...p,
          ...
          entities:
            Array.isArray(p.entities) && p.entities.length > 0
              ? p.entities
              : current.entities,
          accounts: withEntity(p.accounts, current.accounts),
          transactions: withEntity(p.transactions, current.transactions),
          investments: withEntity(p.investments, current.investments),
          students: p.students ?? current.students,
          contributions: p.contributions ?? current.contributions,
          activeEntityId: p.activeEntityId ?? "all",
          display: p.display ?? current.display,
